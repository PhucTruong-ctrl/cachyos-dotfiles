...
run sudo cp "$SCRIPT_DIR/etc/modprobe.d/nvidia.conf" /etc/modprobe.d/nvidia.conf
...
# GTK CSS files are removed before restow because stow cannot overwrite existing files;
# these are regenerated by the stow-managed config after linking.
...
if ! run systemctl --user restart quickshell.service; then
